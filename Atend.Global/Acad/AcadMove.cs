    ...
    public class AcadMove
    {
        ...
        private static void MoveConductor(ObjectId Consol, ObjectId Conductor, Point3d LastCenterPoint)
        {
            ...
            if (AllowToMove)
            {
                ...
                // compute center point of consol
                //ed.WriteMessage("MoveConductor 2 \n");
                Polyline consolEntity = (Polyline)Atend.Global.Acad.UAcad.GetEntityByObjectID(Consol);
                if (consolEntity != null)
                {
                    //ed.WriteMessage("MoveConductor 3 \n");
                    ConsolCenterPoint = Atend.Global.Acad.UAcad.CenterOfEntity(consolEntity);
                }
        ...
        private static void MoveJumper(ObjectId Conductor, ObjectId Jumper, Line LastCondutor)
        {
            ...
            {
                Reset();
                //ed.writeMessage("move jumper 1\n");
                //Point2d JumperCenterPoint = Point2d.Origin;
                //Line JumperEntity = (Line)Atend.Global.Acad.UAcad.GetEntityByObjectID(Jumper);
                ////ed.writeMessage("startpoint:{0},endpoint: {1}\n", JumperEntity.StartPoint, JumperEntity.EndPoint); 
                //Database db = HostApplicationServices.WorkingDatabase;
                Database db = Application.DocumentManager.MdiActiveDocument.Database;
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    //ed.writeMessage("move jumper 2\n");
                    Entity ent = (Entity)tr.GetObject(Jumper, OpenMode.ForWrite);
                    //Line LineEntity = (Line)ent;
                    Line LineEntity = ent as Line;
                    if (LineEntity != null)
                    {
                        Line ConductorLine = (Line)Atend.Global.Acad.UAcad.GetEntityByObjectID(Conductor);
                        //ed.writeMessage("startconductor: {0},  endconductor{1}\n", ConductorLine.StartPoint, ConductorLine.EndPoint);
                    ...
                    {
                        Polyline pLineEntity = ent as Polyline;
                        if (pLineEntity != null)
                        {
                            Line ConductorLine = (Line)Atend.Global.Acad.UAcad.GetEntityByObjectID(Conductor);
                            LineSegment3d LineSeg = new LineSegment3d(ConductorLine.StartPoint, ConductorLine.EndPoint);
                            LineSegment3d LineSegLast = new LineSegment3d(LastCondutor.StartPoint, LastCondutor.EndPoint);
                            bool sw1 = LineSegLast.IsOn(pLineEntity.GetPoint3dAt(0));
                            bool sw2 = LineSegLast.IsOn(pLineEntity.GetPoint3dAt(1));
                            Point3d p;
                            if (sw1)
                            {
                                p = LineSeg.GetClosestPointTo(pLineEntity.GetPoint3dAt(0)).Point;
                                pLineEntity.SetPointAt(0, new Point2d(p.X, p.Y));
                            }
                            else
                            {
                                p = LineSeg.GetClosestPointTo(pLineEntity.GetPoint3dAt(1)).Point;
                                pLineEntity.SetPointAt(1, new Point2d(p.X, p.Y));
                            }
                        }
                        ed.WriteMessage("line was polyline \n");
                    }
                    tr.Commit();
                    ...
                    if (Atend.Global.Acad.UAcad.IsInsideCurve(ParentCurve, CenterGround) == false)
                    {
                        ...
                        foreach (ObjectId _collect in _Collection)
                        {
                            ...
                            if (l != null)
                            {
                                ...
                                AllowToMove = true;
                                Atend.Global.Acad.AcadMove.EntityInfo = Atend.Base.Acad.AT_INFO.SelectBySelectedObjectId(_collect);
                                Atend.Global.Acad.AcadMove.EntitySub = Atend.Base.Acad.AT_SUB.SelectBySelectedObjectId(_collect);
