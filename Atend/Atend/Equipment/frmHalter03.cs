...
{
    public partial class frmHalter03 : Form
    {
        ...
        Guid SelectedHalterXCode = Guid.Empty;
        ...
        private void Reset()
        {
            SelectedHalterXCode = Guid.Empty;
        ...
        {
            txtName.Focus();
            Atend.Base.Equipment.EHalter Halter = new Atend.Base.Equipment.EHalter();
            Halter.ProductCode = Atend.Control.Common.selectedProductCode;
            //Halter.Amper = Convert.ToDouble(txtAmper.Text);
            //Halter.Comment = txtComment.Text;
            Halter.Name = txtName.Text;
            Halter.X = Convert.ToDouble(txtX.Text);
            Halter.Y = Convert.ToDouble(txtY.Text);
            Halter.IsDefault = IsDefault;
            Halter.Power = Convert.ToDouble(txtPower.Text);
            Halter.Code = Code;
            //Equipment
            ...
            Halter.EquipmentList = EPackageProduct;
            ...
            Halter.OperationList = EOperation;
            if (SelectedHalterXCode == Guid.Empty)
            {
                if (Halter.InsertX())
                    Reset();
                else
            ...
            {
                Halter.XCode = new Guid(SelectedHalterXCode.ToString());
                if (Halter.UpdateX())
                    Reset();
                else
        ...
        private void Delete()
        {
            ...
            if (!Atend.Global.Utility.UBinding.ExistInSubEquip(SelectedHalterXCode, out name))
            {
            ...
            //Atend.Base.Equipment.EProductPackage _ProductPackage = Atend.Base.Equipment.EProductPackage.SelectByXCode(SelectedHalterXCode);
            ...
            if (MessageBox.Show("آیا از حذف کردن اطلاعات اطمینان دارید؟", "حذف", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                if (SelectedHalterXCode != Guid.Empty)
                {
                    if (Atend.Base.Equipment.EHalter.DeleteX(SelectedHalterXCode))
                        Reset();
                    else
        ...
        {
            Atend.Base.Equipment.EHalter Hl = Atend.Base.Equipment.EHalter.SelectByXCode(XCode);
            //Atend.Base.Base.BProduct product = Atend.Base.Base.BProduct.Select_ByCode(key.ProductCode);
            Atend.Control.Common.selectedProductCode = Hl.ProductCode;

            SelectProduct();
            txtName.Text = Hl.Name;
            txtX.Text = Math.Round(Hl.X,4).ToString();
            txtY.Text = Math.Round(Hl.Y,4).ToString();
            txtPower.Text = Math.Round(Hl.Power,4).ToString();
            //txtAmper.Text = Convert.ToString(Math.Round(Fl.Amper, 2));
            //txtComment.Text = Fl.Comment;
            tsbIsDefault.Checked = Hl.IsDefault;
            SelectedHalterXCode = new Guid(XCode.ToString());
            Code = Hl.Code;
            BindTreeViwAndGrid();
        }
        ...
        private void BindTreeViwAndGrid()
        {
            ...
            for (int i = 0; i < Atend.Base.Equipment.EHalter.nodeKeysEPackageX.Count; i++)
            {
                string s = Atend.Base.Equipment.EHalter.nodeKeysEPackageX[i].ToString();
                foreach (TreeNode rootnode in tvEquipment.Nodes)
                {
                    foreach (TreeNode chileNode in rootnode.Nodes)
                    {
                        if (Atend.Base.Equipment.EHalter.nodeTypeEPackageX[i].ToString() == chileNode.Name.ToString())
                        {
                            if (chileNode.Tag.ToString() == s)
                            {
                                ...
                                gvSelectedEquipment.Rows[gvSelectedEquipment.Rows.Count - 1].Cells[2].Value = Atend.Base.Equipment.EHalter.nodeCountEPackageX[i].ToString();
            ...
            ClearCheckAndGrid(tvOperation, gvOperation);
            gvOperation.Refresh();
            for (int i = 0; i < Atend.Base.Equipment.EHalter.nodeKeys.Count; i++)
            {
                Atend.Base.Equipment.EOperation Operation = ((Atend.Base.Equipment.EOperation)Atend.Base.Equipment.EHalter.nodeKeys[i]);
        ...
        private bool CheckStatuseOfAccessChangeDefault()
        {
            ...
            {
                if (SelectedHalterXCode == Guid.Empty && IsDefault)
                {
                ...
                {
                    Atend.Base.Equipment.EHalter Equip = Atend.Base.Equipment.EHalter.SelectByXCode(SelectedHalterXCode);
        ...
        {
            frmHalterSearch03 _frmHalterSearch03 = new frmHalterSearch03(this);
            Autodesk.AutoCAD.ApplicationServices.Application.ShowModalDialog(_frmHalterSearch03);
        }
        ...
        {
            Atend.Base.frmProductSearch frmproductSearch = new Atend.Base.frmProductSearch(Atend.Control.Enum.ProductType.Halter);
        ...
        private void tsbShare_Click(object sender, EventArgs e)
        {
            ...
            if (SelectedHalterXCode != Guid.Empty)
            {
                if (Atend.Base.Equipment.EContainerPackage.ShareOnServer(Convert.ToInt32(Atend.Control.Enum.ProductType.Halter), SelectedHalterXCode))
                {
                    Atend.Base.Equipment.EHalter Halter = Atend.Base.Equipment.EHalter.SelectByXCode(SelectedHalterXCode);
                    Code = Halter.Code;
            ...
            //if (SelectedHalterXCode != Guid.Empty)
            //{
            //    if (Atend.Base.Equipment.EHalter.ShareOnServer(SelectedHalterXCode))
            //    {
            //        Atend.Base.Equipment.EHalter h1 = Atend.Base.Equipment.EHalter.SelectByXCode(SelectedHalterXCode);
            //        Code = h1.Code;
            //        MessageBox.Show("به اشتراک گذاری با موفقیت انجام شد");
            //    }
            //    else
