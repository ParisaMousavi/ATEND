...
{
    public class AcDrawDisConnector02
    {
        bool _UseAccess;
        ...
        public class DrawDisconnectorJig : DrawJig
        {
            ...
            public List<Entity> GetDemo(Point3d StartPoint, Point3d EndPoint)
            {
                ...
                Entities.Add(CreateLine(new Point3d(BasePoint.X - 5, BasePoint.Y, BasePoint.Z), new Point3d(BasePoint.X - 2, BasePoint.Y, BasePoint.Z), (int)Atend.Control.Enum.ProductType.Disconnector, 0, 0));
                Entities.Add(CreateLine(new Point3d(BasePoint.X + 5, BasePoint.Y, BasePoint.Z), new Point3d(BasePoint.X + 2, BasePoint.Y, BasePoint.Z), (int)Atend.Control.Enum.ProductType.Disconnector, 0, 0));
                ...
                Entities.Add(CreateLine(new Point3d(BasePoint.X + 2, BasePoint.Y + 0.5, BasePoint.Z), new Point3d(BasePoint.X + 2, BasePoint.Y - 0.5, BasePoint.Z), (int)Atend.Control.Enum.ProductType.Disconnector, 0, 0));
        ...
        /// <param name="EndEntity"></param>
        /// <param name="CellData"></param>
        /// <returns></returns>
        public object DrawDisconnectorCell(Entity StartEntity, Entity EndEntity, DataRow CellData)
        {
            Editor ed = Application.DocumentManager.MdiActiveDocument.Editor;
            ObjectId DrawnDisconnector = ObjectId.Null;
            System.Data.DataTable ParentList = Atend.Global.Acad.UAcad.DetermineParent((int)Atend.Control.Enum.ProductType.Disconnector);
            ...
            if (StartEntity != null)
            {
                Atend.Base.Acad.AT_INFO Info1 = Atend.Base.Acad.AT_INFO.SelectBySelectedObjectId(StartEntity.ObjectId);
                DataRow[] drs = ParentList.Select(string.Format("SoftwareCode={0}", Info1.NodeType));
                if (drs.Length != 0)
                {
                    //PromptEntityOptions peo2 = new PromptEntityOptions("Select second node:");
                    ...
                    if (EndEntity != null)
                    {
                        Atend.Base.Acad.AT_INFO Info2 = Atend.Base.Acad.AT_INFO.SelectBySelectedObjectId(EndEntity.ObjectId);
                        DataRow[] drs1 = ParentList.Select(string.Format("SoftwareCode={0}", Info2.NodeType));
                        if (drs1.Length != 0 && Info1.ParentCode == Info2.ParentCode)
                        {
                            ...
                            DrawDisconnectorJig _DrawDisconnectorJig = new DrawDisconnectorJig(MyScale);
                            List<Entity> ents = _DrawDisconnectorJig.GetDemo(
                                Atend.Global.Acad.UAcad.CenterOfEntity(StartEntity),
                                Atend.Global.Acad.UAcad.CenterOfEntity(EndEntity));
                            ...
                            if (CellData != null)
                            {
                                ...
                                foreach (Entity ent in ents)
                                {
                                    ...
                                    if (TerminalOI == ObjectId.Null)
                                    {
                                        if (KetOI == ObjectId.Null)
                                        {
                                            //ed.WriteMessage("TerminalOI == null\n");
                                            Atend.Base.Acad.AT_INFO groupInfo = new Atend.Base.Acad.AT_INFO(NOI);
                                            groupInfo.NodeCode = CellData["NodeCode"].ToString();
                                            groupInfo.ParentCode = CellData["ParentCode"].ToString();
                                            groupInfo.NodeType = (int)Atend.Control.Enum.ProductType.Disconnector;
                                            groupInfo.ProductCode = Convert.ToInt32(CellData["ProductCode"]);
                                            groupInfo.Insert();
                                        }
                                        else
                                        {
                                ...
                                ObjectId GOI = Atend.Global.Acad.Global.MakeGroup("Disconnector-" + CellData["NodeCode"].ToString(), OIC);

                                if (GOI != ObjectId.Null)
                                {
                                    DrawnDisconnector = GOI;
                                    Atend.Base.Acad.AT_INFO groupInfo1 = new Atend.Base.Acad.AT_INFO(GOI);
                                    groupInfo1.NodeCode = CellData["NodeCode"].ToString();
                                    groupInfo1.ParentCode = CellData["ParentCode"].ToString();
                                    groupInfo1.NodeType = (int)Atend.Control.Enum.ProductType.Disconnector;
                                    groupInfo1.ProductCode = Convert.ToInt32(CellData["ProductCode"]);
                                    groupInfo1.Insert();


                                    ObjectId TextOi = Atend.Global.Acad.UAcad.DrawEntityOnScreen(
                                        Atend.Global.Acad.UAcad.WriteNote("Disconnector",
                                        _DrawDisconnectorJig.MyCenterPoint
                                        , MyCommentScale), Atend.Control.Enum.AutoCadLayerName.GENERAL.ToString()
                                        );
                                    ...
                                    Atend.Base.Acad.AT_INFO textInfo = new Atend.Base.Acad.AT_INFO(TextOi);
                                    textInfo.NodeCode = "";
                                    textInfo.ParentCode = CellData["ParentCode"].ToString();
                                    textInfo.NodeType = (int)Atend.Control.Enum.ProductType.Comment;
                                    textInfo.ProductCode = 0;
                                    textInfo.Insert();

                                    Atend.Base.Acad.AT_SUB groupSub = new Atend.Base.Acad.AT_SUB(GOI);
                                    groupSub.SubIdCollection.Add(StartEntity.ObjectId);
                                    groupSub.SubIdCollection.Add(EndEntity.ObjectId);
                                    groupSub.SubIdCollection.Add(TextOi);
                                    groupSub.Insert();

                                    Atend.Base.Acad.AT_SUB.AddToAT_SUB(GOI, StartEntity.ObjectId);
                                    Atend.Base.Acad.AT_SUB.AddToAT_SUB(GOI, EndEntity.ObjectId);
                                }
