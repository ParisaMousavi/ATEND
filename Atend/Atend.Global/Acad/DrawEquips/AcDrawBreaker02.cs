...
namespace Atend.Global.Acad.DrawEquips
{
    public class AcDrawBreaker02
    {
        ...
        public object DrawBreakerCell(Entity StartEntity, Entity EndEntity, DataRow CellData)
        {
            Editor ed = Application.DocumentManager.MdiActiveDocument.Editor;
            //ed.WriteMessage("DRaw BREAKER \n");
            ObjectId DrawnBreaker = ObjectId.Null;
            System.Data.DataTable ParentList = Atend.Global.Acad.UAcad.DetermineParent((int)Atend.Control.Enum.ProductType.Breaker);
            //double V1 = 0;
            //double V2 = 0;
                            ...
                            if (CellData != null)
                            {
                                ...
                                foreach (Entity ent in ents)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        groupInfo.ParentCode = CellData["ParentCode"].ToString();
                                        groupInfo.NodeType = (int)Atend.Control.Enum.ProductType.Terminal;
                                        groupInfo.ProductCode = Convert.ToInt32(CellData["ParentCode"]);
                                        groupInfo.Insert();
                                    }
                                }

                                ObjectId GOI = Atend.Global.Acad.Global.MakeGroup("Breaker-" + CellData["NodeCode"].ToString(), OIC);
                                //ed.WriteMessage("----<<<<<----GOI:{0}\n",GOI);
                                if (GOI != ObjectId.Null)
                                {
                                    ...
                                    ObjectId TextOi = Atend.Global.Acad.UAcad.DrawEntityOnScreen(
                                        Atend.Global.Acad.UAcad.WriteNote("Breaker",
                                        _DrawBreakerJig.MyCenterPoint
                                        , MyCommentScale), Atend.Control.Enum.AutoCadLayerName.GENERAL.ToString()
                                        );
